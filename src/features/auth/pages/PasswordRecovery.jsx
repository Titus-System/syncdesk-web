...
export default function PasswordRecovery() {
  ...
  const currentStep = token ? 3 : step
  ...
  async function handleResetPassword() {
    ...
    try {
      ...
      setErrorMessage(error.response?.data?.message || 'Erro ao redefinir. Token expirado.')
    }
  }
          ...
          {currentStep === 1 && (
            <form className="space-y-6 text-center" onSubmit={handleRequestRecovery}>
              ...
              <button
                type="submit"
                disabled={isLoading}
                className={`w-full ${colors.primary} text-white py-4 rounded-xl font-bold flex items-center justify-center gap-2 transition-all ${colors.hover} disabled:opacity-50`}
              >
          ...
          {currentStep === 2 && (
            <div className="space-y-6 text-center">
              ...
              <button
                type="button"
                onClick={handleBackToLogin}
                className={`w-full ${colors.primary} text-white py-4 rounded-xl font-bold ${colors.hover}`}
              >
