        ...
        <div className="flex-1 overflow-y-auto p-6 lg:p-8">
          <div className="w-full max-w-3xl mx-auto">
            ...
            <div className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border-subtle)] shadow-sm p-6 mb-4">
              ...
              <form onSubmit={handleSalvar} className="flex flex-col gap-5">
                ...
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 rounded-full bg-orange-100 text-orange-700 flex items-center justify-center text-xl font-bold shrink-0 shadow-sm">
                    {initials}
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" value={nome} onChange={(e) => setNome(e.target.value)}
                      ...
                      className="w-full px-4 py-3 border border-[var(--border-default)] rounded-xl text-sm outline-none focus:border-[var(--accent)] transition-all" />
                  </div>
                  <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <select defaultValue="pt-BR"
                    className="w-full px-4 py-2 border border-[var(--border-default)] rounded-xl text-sm outline-none focus:border-[var(--accent)] transition-all appearance-none">
            ...
            <div className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border-subtle)] shadow-sm p-6 mb-10">
              ...
              <div className="mt-5">
                <button type="button"
                  className="text-xs font-bold text-[var(--accent-text)] hover:text-[#9a2f0d] border border-[var(--accent)]/30 rounded-xl px-4 py-2 hover:bg-orange-50 transition-all">
                  Alterar Senha
                </button>
              </div>
            </div>
...
function ToggleSwitch({ defaultOn = false }) {
  ...
  return (
    <button type="button" onClick={() => setOn((v) => !v)}
      className={`relative w-10 h-5 rounded-full transition-colors shrink-0 ${on ? 'bg-[var(--accent)]' : 'bg-gray-200'}`}>
      <span className={`absolute top-0.5 left-0.5 w-4 h-4 bg-[var(--bg-card)] rounded-full shadow transition-transform ${on ? 'translate-x-5' : ''}`} />
    </button>
  )
}
